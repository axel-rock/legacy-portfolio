...
Helper.createTextElement = function(id, parent, defaultStyle) {
	...
	var element = Helper.createElement({id: id, parent: parent})
	var lines = textContent.split('<br/>')
	for (var i = 0; i < lines.length; i++) {
		element.innerHTML += '<span>' + lines[i] + '</span>'
	}
	...
	if (dc[id + '_STYLE'].indexOf('font-size') < 0 && lines.length > 1) {
		$('#' + id).bigtext()
	}
	return element
}
